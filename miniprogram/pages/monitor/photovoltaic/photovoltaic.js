...
Page({
  ...
  getLatestData(){ // 获取最新实时数据
    let that = this;
    util.wxRequestGet("/prod-api/sps/solar/power/getTodayRealTimeData", "加载中...", {}, 'application/json', function(res) {
      if(res.code==200){
  ...
  getTodayPowerData(){ //获得今日功率趋势
    let that = this;
    util.wxRequestGet("/prod-api/sps/solar/power/getTodayPowerData", "加载中...", {}, 'application/json', function(res) {
      if(res.code==200){
